...
Event::where('status' , 0)
    ...
    public function tceVagas($ano = null){
        ...
        $programas = array('1','2','3','4','12');
            foreach($programas as $programa){
                $vagas[$programa] = \App\Models\Turma::whereYear('data_inicio',$ano)->where('programa',$programa)->whereIn('status',['iniciada','encerrada'])->sum('vagas');
                $turmas[$programa] = \App\Models\Turma::whereYear('data_inicio',$ano)->where('programa',$programa)->whereIn('status',['iniciada','encerrada'])->count();
